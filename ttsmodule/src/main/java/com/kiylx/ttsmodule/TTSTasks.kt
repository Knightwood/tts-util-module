...
abstract class TTSTask(
    ...
    init {
        ...
        // task is begun.
        ttsEngineName = AppUtils.getAppPackageName()
    }

    fun displayMessage(string: String, long: Boolean) {
            if (long) ToastUtils.showShort(string)
            else ToastUtils.showShort(string)
    }

    fun displayMessage(id: Int, long: Boolean) {
        if (long) ToastUtils.showShort(id)
        else ToastUtils.showShort(id)
    ...
    protected open fun performValidationChecks(): Int {
        ...
        if (inputSource.isSourceAvailable(ctx)) {
            inputStream = inputSource.openInputStream(ctx)
            inputSize = inputSource.getSize(ctx)
        }
    ...
    private fun enqueueNextInput(): Boolean {
        ...
        enqueueFromInfo(utteranceInfo)//朗读文字
    ...
    /**
     * 这个方法的调用，最终调用了[enqueueText]方法去朗读文字
     */
    ...
    open fun finish(success: Boolean): Boolean {
        ...
        if (inputFiltered > 0) {
            val message = ctx.getString(
                ...
                inputFiltered, ctx.resources.getQuantityString(
...
class ReadInputTask(
    ...
    override fun finish(success: Boolean): Boolean {
        TTSHolder.getTTS().releaseAudioFocus()
...
class FileSynthesisTask(
    ...
    override fun performValidationChecks(): Int {
        ...
        if (!outDirectory.canWrite(ctx)) return UNWRITABLE_OUT_DIR
