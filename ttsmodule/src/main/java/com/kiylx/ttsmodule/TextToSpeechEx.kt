...
/**
 * check put in languages ,if some one is acceptable,return it.
 * if all put in can not use , return null
 */
fun TextToSpeech.checkLanguages(list: List<Locale>): Locale? {
    val result: Locale? = list.find {
        val result = this.findAcceptableTTSLanguage(it)
        result != null
    }
    return result
}
